...
load_model_summaries <- function(sex, base_path) {
  ...
  if (sex == "male") {
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Suic_BRFFS", paste0("modM", i, "lincovall.Rdata"))
        load(model_path)
    ...
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Suic_BRFFS", paste0("modF", i, "lincovall.Rdata"))
        load(model_path)
...
for (var in covariates_to_plot) {
  ...
  out <- paste0("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/7_22/Results/Suic_BRFFS/_cred_int_",
