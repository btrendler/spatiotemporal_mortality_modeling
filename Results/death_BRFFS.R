...
load_model_summaries <- function(sex, base_path) {
  ...
  if (sex == "male") {
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Death_BRFFS", paste0("modM", i, "lincovall.Rdata"))
        load(model_path)
    ...
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Death_BRFFS", paste0("modF", i, "lincovall.Rdata"))
        load(model_path)
...
base_path <- "/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/6_5"
...
covariates_to_plot <- c("MentalHealthDays", "(Intercept)", "alpha", "tau2.int", "tau2.slo", "rho.int", "rho.slo")
pretty_names <- c(
  "MentalHealthDays" = "Coefficient for Mental Health Days",
  "(Intercept)" = "Intercept",
...
y_label_for <- function(var) {
  switch(var,
    "MentalHealthDays"  = expression(beta[1]),
...
for (var in covariates_to_plot) {
  ...
  out <- paste0("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/7_22/Results/Death_BRFFS/_cred_int_",
