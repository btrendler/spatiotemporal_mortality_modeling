...
setwd("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/6_5/male/Suic_BRFFS")
...
# Save each plot as a PNG file
  png_filename <- paste0("_Final_Plots/AlphaPlusDeltaM_", i, ".png")
  ...
  model_name <- paste0("modM", i, "lincovall.Rdata")
  ...
  alpha <- mean(modMLincov$samples$alpha)
  rep_alpha <- rep(alpha, ncol(modMLincov$samples$delta))
  time <- rep_alpha + base::apply(modMLincov$samples$delta, 2, mean)
  ...
  p <- ggplot(deltas_map) +
        ...
        labs(title = bquote(alpha~"+"~delta[k]~"for Males, Ages "~.(age_labels[as.character(i)])), fill = NULL) +
        theme_minimal()
