...
setwd("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/6_5/male/Death_BRFFS")
...
# beta_cred_int <- beta_cred_int |> filter(AgeGroup > 2)

# f_beta_cred_int_list <- readRDS("../../female/Suic_DepRate/Diagnostics/beta_cred_int.rds")
# f_beta_cred_int <- do.call(rbind, f_beta_cred_int_list)
# f_beta_cred_int <- f_beta_cred_int |> filter(AgeGroup > 2)

# ymin <- min(min(beta_cred_int$Q2.5), min(f_beta_cred_int$Q2.5))
# ymax <- max(max(beta_cred_int$Q97.5), max(f_beta_cred_int$Q97.5))
...
for (i in 1:18) {
  ...
  # # Save map
  # jpeg(paste("Diagnostics/delta_by_county_M", i, ".jpeg", sep = ""))
  # print(
  #   ggplot(deltas_map) +
  ...
  # Save map
  png_filename= paste("_Final_Plots/phi_by_county_M", i, ".png", sep = "")
  png(filename = png_filename, width = 2000, height = 1400, res = 300)
  print(
    ggplot(phis_map) +
      geom_sf(aes(fill = value), color = NA) +
      scale_fill_viridis_c(option = "D", na.value = "grey90") +
      labs(title = bquote("Mean Spatial Effect for Males, Age "~.(age_labels[as.character(i)])), fill = NULL) +
      theme_minimal()
  )
  dev.off()
}
...
ggplot(beta_cred_int, aes(x = AgeGroup, y = Mean)) +
  ...
  geom_errorbar(aes(ymin = Q2.5, ymax = Q97.5), width = 0.3, color = "deepskyblue4") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  labs(
    title = "Effect of DepRate on Y_suicide for males by Age Group (Posterior Mean ± 95% CI)",
    x = "Age Group",
    y = "Posterior Mean"
  ) +
  ylim(ymin, ymax) +
