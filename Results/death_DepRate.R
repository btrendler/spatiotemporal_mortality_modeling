...
load_model_summaries <- function(sex, base_path) {
  ...
  if (sex == "male") {
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Death_DepRate", paste0("modM", i, "lincovall.Rdata"))
        load(model_path)
    ...
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Death_DepRate", paste0("modF", i, "lincovall.Rdata"))
        load(model_path)
...
covariates_to_plot <- c("dep_rate", "(Intercept)", "alpha", "tau2.int", "tau2.slo", "rho.int", "rho.slo")
pretty_names <- c(
  "dep_rate" = "Coefficient for Depression Rate",
  "(Intercept)" = "Intercept",
...
for (var in covariates_to_plot) {
  ...
  out <- paste0("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/7_22/Results/Death_DepRate/_cred_int_",
