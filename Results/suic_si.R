...
load_model_summaries <- function(sex, base_path) {
  ...
  if (sex == "male") {
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Suic_SIRate", paste0("modM", i, "lincovall.Rdata"))
        load(model_path)
    ...
    for (i in 1:18) {
        model_path <- file.path(base_path, sex, "Suic_SIRate", paste0("modF", i, "lincovall.Rdata"))
        load(model_path)
...
# Updated plotting function that shows both sexes
...
# Save plots for each covariate
for (var in covariates_to_plot) {
  jpeg(paste0("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/7_22/Results/Suic_SIRate/_cred_int_", gsub("[()]", "", var), "_male_female.png"), width = 700, height = 600, quality = 100)
  print(cred_int_plot_dual(var))
  dev.off()
}
