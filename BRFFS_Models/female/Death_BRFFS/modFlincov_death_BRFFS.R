...
for(i in 1:18) {
  ...
  offset <- forMod$pop
  form <- total_deaths ~ MentalHealthDays
  ...
  beta_row <- as.data.frame(t(modFLincov$summary.results["MentalHealthDays", ]))
  beta_row$AgeGroup <- i
  ...
  plot(modFLincov$samples$beta[,2], type="l", main="MentalHealthDays")
