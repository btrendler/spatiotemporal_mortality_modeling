...
m_beta_cred_int_list <- readRDS("../../male/Death_DepRate/Diagnostics/beta_cred_int.rds")
...
ggplot(beta_cred_int, aes(x = AgeGroup, y = Mean)) +
  ...
  labs(
    title = "Effect of depRate on total_deaths for females by Age Group (Posterior Mean ± 95% CI)",
    x = "Age Group",
    y = "Posterior Mean"
  ) +
  ylim(ymin, ymax) +
