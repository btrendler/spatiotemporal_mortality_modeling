...
setwd("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/7_22/MHA/female/Suic_PsychRate")
...
m_beta_cred_int_list <- readRDS("../../male/Suic_PsychRate/Diagnostics/beta_cred_int.rds")
m_beta_cred_int <- do.call(rbind, m_beta_cred_int_list)
m_beta_cred_int <- m_beta_cred_int |> filter(AgeGroup > 2)
...
ggplot(beta_cred_int, aes(x = AgeGroup, y = Mean)) +
  ...
  labs(
    title = "Effect of PsychRate on Y_suicide for females by Age Group (Posterior Mean ± 95% CI)",
    x = "Age Group",
    y = "Posterior Mean"
  ) +
  ylim(ymin, ymax) +
