...
setwd("/yunity/bt327/spatmort/brigg_trendler/MentalHealth/rscripts/modeling/models/7_22/MHA/male/Suic_PsychRate")
...
f_beta_cred_int_list <- readRDS("../../female/Suic_PsychRate/Diagnostics/beta_cred_int.rds")
...
for (i in 1:18) {
  ...
  # Save map
  jpeg(paste("Diagnostics/phi_by_county_M", i, ".jpeg", sep = ""))
  print(
    ggplot(phis_map) +
      geom_sf(aes(fill = value), color = NA) +
...
ggplot(beta_cred_int, aes(x = AgeGroup, y = Mean)) +
  ...
  labs(
    title = "Effect of PsychRate on Y_suicide for males by Age Group (Posterior Mean ± 95% CI)",
    x = "Age Group",
    y = "Posterior Mean"
  ) +
  ylim(ymin, ymax) +
